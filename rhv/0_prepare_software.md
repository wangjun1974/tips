...
# 同步软件仓库
/usr/bin/nohup /bin/bash RHV4_4_repo_sync_up.sh &

# 同步完之后把 /repos/rhv44 目录打包，下载作为离线时使用的软件仓库

```

### Yum 仓库和镜像仓库准备

RHV Manager 在部署时需要访问镜像仓库，在一般的部署下，这个镜像仓库会部署在 rhvm 上

```
# 1. 安装时需使用 rhel-8.3-x86_64-dvd.iso 作为操作系统的安装介质
# 这个 ISO 可在红帽官网下载
