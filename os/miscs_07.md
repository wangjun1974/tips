...
LC_ALL=en_US.UTF-8 virt-install --name=master1 --vcpus=4 --memory=16384 --disk size=100,bus=virtio,format=qcow2 --network bridge=virbr0,model=virtio --os-variant=rhel8.0 --boot menu=on --console pty,target_type=serial --location=/var/lib/libvirt/images/rhcos-live.x86_64.iso --extra-args 'console=tty0 console=ttyS0 rw coreos.liveiso=rhcos-9.6.20250826-1 igniton.firstboot ignition.platform=baremetal ip=10.120.88.125::10.120.88.1:255.255.255.0:master1.ocp.ap.vwg:ens3:none:10.120.88.123 coreos.inst.install_dev=/dev/vda coreos.inst.ignition_url=http://helper.ocp.ap.vwg:8080/ocp/ignition/master.ign coreos.inst.insecure'

LC_ALL=en_US.UTF-8 virt-install --name=master2 --vcpus=4 --memory=16384 --disk size=100,bus=virtio,format=qcow2 --network bridge=virbr0,model=virtio --os-variant=rhel8.0 --boot menu=on --console pty,target_type=serial --location=/var/lib/libvirt/images/rhcos-live.x86_64.iso --extra-args 'console=tty0 console=ttyS0 rw coreos.liveiso=rhcos-9.6.20250826-1 igniton.firstboot ignition.platform=baremetal ip=10.120.88.126::10.120.88.1:255.255.255.0:master2.ocp.ap.vwg:ens3:none:10.120.88.123 coreos.inst.install_dev=/dev/vda coreos.inst.ignition_url=http://helper.ocp.ap.vwg:8080/ocp/ignition/master.ign coreos.inst.insecure'

LC_ALL=en_US.UTF-8  virt-install --name=master3 --vcpus=4 --memory=16384 --disk size=100,bus=virtio,format=qcow2 --network bridge=virbr0,model=virtio --os-variant=rhel8.0 --boot menu=on --console pty,target_type=serial --location=/var/lib/libvirt/images/rhcos-live.x86_64.iso --extra-args 'console=tty0 console=ttyS0 rw coreos.liveiso=rhcos-9.6.20250826-1 igniton.firstboot ignition.platform=baremetal ip=10.120.88.127::10.120.88.1:255.255.255.0:master3.ocp.ap.vwg:ens3:none:10.120.88.123 coreos.inst.install_dev=/dev/vda coreos.inst.ignition_url=http://helper.ocp.ap.vwg:8080/ocp/ignition/master.ign coreos.inst.insecure'
...
https://github.com/coulof/dell-csi-static-pv/blob/main/ingest-static-pv.sh
```
...
### 检查 vmi 的 audit 日志看跟 create/delete 有关的日志 
```
oc debug node/m1-ocp4test.ocp4.example.com -q -- chroot /host grep '"resource":"virtualmachineinstances"' /var/log/kube-apiserver/audit.log | grep '"name":"rhel9-jwang-audit-01"' | grep -Ev '"verb":"update"|"verb":"patch"'
...
### 将备份拷贝到本地
mkdir -p /var/www/html/backup/2026-03-05

### 将备份拷贝到本地
ssh -i /data/ocp-cluster/ocp/ssh-key/id_rsa core@master1.ocp.ap.vwg sudo chmod 644 /home/core/assets/backup/*
scp -i /data/ocp-cluster/ocp/ssh-key/id_rsa core@master1.ocp.ap.vwg:/home/core/assets/backup/* /var/www/html/backup/2026-03-05/
...
master2.ocp.ap.vwg   NotReady   control-plane,master   4d17h   v1.33.6 <==
master3.ocp.ap.vwg   NotReady   control-plane,master   4d16h   v1.33.6 <==
